...
./build/wss_server --port "$PORT" --cert "$CERT_FILE" --key "$KEY_FILE" \
  --http-port 0 --no-log-file > "$OUT_DIR/server_text.log" 2>&1 &
SERVER_PID=$!
cleanup() {
  # 退出时总是清理服务端进程，防止残留。
...
csv_append() {
  { flock -x 9; echo "$1" >> "$CSV"; } 9>"$OUT_DIR/.text_rtt.csv.lock"
}
...
run_one() {
  ...
  if ./build/wss_client --host "$HOST" --port "$PORT" --ca "$CA_FILE" --text "bench-${r}-${c}" >/dev/null 2>&1; then
    ...
    elapsed=$((end - start))
    csv_append "${r},${c},${elapsed},1"
  else
    end=$(date +%s%3N)
    elapsed=$((end - start))
    csv_append "${r},${c},${elapsed},0"
  fi
}

export -f run_one csv_append
export HOST PORT CA_FILE CSV OUT_DIR
